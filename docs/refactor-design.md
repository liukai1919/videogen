...
| C（已实现，画布 lite）· 工作台 UI | `/workspace` 三区布局：左侧项目/Skill/资产，中间产物区（jobs+renders 合流网格，图片可存资产），右侧 Agent 对话（会话管理、工具调用内联展示）；真无限画布归阶段 E |
| D（已实现）· 配音与成片 | tts 能力出解说音轨；内建 `compose` 能力（FFmpeg，CPU 不占 GPU 闸门）把渲染+配音+SRT 拼成成片，走同一条任务队列；Agent 新增 `compose_video` 工具，`write_storyboard` 直接返回按渲染时间轴对齐的 `narration_srt`——"写分镜→渲染→配音→合成"四个工具即一条成片链 |
